 ...
 * and explicit emotional wording wins over punctuation alone.
 */
function detectVocalState(text: string): VocalState {
    const t = text.toLowerCase().trim();

    // Explicit semantic signals first. This prevents e.g. "sorry to hear that"
    // from being mistaken for embarrassment.
    if (/\b(sorry to hear|i'm sorry|i am sorry|miss you|missed you|sad|heartbroken)\b|難過|傷心|心疼|想你|好難過/.test(t)) return "sad";
    if (/\b(love you|love ya|dear|sweetheart|miss you so much)\b|愛你|親愛的|好想你/.test(t)) return "affectionate";
    if (/\b(furious|pissed|enraged|hate this|i hate)\b|氣死|憤怒|暴怒|討厭死/.test(t)) return "angry";
    if (/\b(ugh|annoying|annoyed|frustrated|whatever)\b|煩死|無語|煩人|受不了/.test(t)) return "annoyed";
    if (/\b(embarrassed|embarrassing|awkward|shy)\b|尷尬|害羞|不好意思/.test(t)) return "embarrassed";
    if (/\b(yeah, right|sure, whatever|great job|obviously|as if)\b|呵呵|行啊|真棒|才怪|是喔/.test(t)) return "sarcastic";
    if (/\b(lol|lmao|haha|hehe|funny|kidding|joking)\b|哈哈|笑死|好笑|開玩笑/.test(t)) return "amused";
    if (/\b(excited|can't wait|amazing|awesome|so happy)\b|期待|太棒|超開心|好興奮/.test(t)) return "excited";
    if (/\b(omg|wow|no way|what\?|you're kidding|are you serious)\b|天啊|什麼|真的假的|不會吧|你認真的嗎/.test(t)) return "surprised";

    // Punctuation is only a weak signal and requires repetition.
