...
async function fetchWithTimeout(url: string, init: RequestInit, timeoutMs = TTS_TIMEOUT_MS): Promise<Response> {
    ...
    try {
        ...
        if (e instanceof DOMException && e.name === "AbortError") throw new Error(`语音合成超时（超过 ${Math.round(timeoutMs / 1000)} 秒无响应）`);
        throw e;
...
async function synthesizeMinimax(text: string, config: VoiceApiConfig, emotion?: string): Promise<Blob | null> {
    ...
    const response = await fetchWithTimeout(`${baseUrl}/t2a_v2`, {
        ...
        body: JSON.stringify({ model: config.model || "speech-01-turbo", text, stream: false, ...(config.languageBoost ? { language_boost: config.languageBoost } : {}), voice_setting: voiceSetting, audio_setting: { sample_rate: 44100, bitrate: 256000, format: "mp3", channel: 1 } }),
    });
...
async function synthesizeF5TTS(text: string, config: VoiceApiConfig): Promise<Blob | null> {
    ...
    const response = await fetchWithTimeout(`${baseUrl}/tts`, {
        ...
        body: JSON.stringify({ text, ref_audio: refAudio, ref_text: config.f5RefText || "", model: config.model || "F5TTS_v1_Base", nfe_step: config.f5NfeStep ?? 32, speed: config.speechSpeed ?? 1, remove_silence: config.f5RemoveSilence ?? false }),
    });
...
export function setTtsVolume(volume: number): void {
    ...
    try { window.localStorage.setItem(TTS_VOLUME_KEY, String(_ttsVolume)); } catch {}
    if (_activeGain) { try { _activeGain.gain.value = _ttsVolume; } catch {} }
    if (_sharedAudio) { try { _sharedAudio.volume = _ttsVolume; } catch {} }
}
export function setCallAudioSessionActive(active: boolean): void {
    _callAudioSessionActive = active;
    if (!active && _audioCtx && !_activeGain) { try { void _audioCtx.suspend(); } catch {} }
}
...
function getSharedAudio(): HTMLAudioElement {
    if (!_sharedAudio) { _sharedAudio = new Audio(); _sharedAudio.setAttribute("playsinline", ""); }
    return _sharedAudio;
}
function silentWavUrl(): string {
    const numSamples = 16;
    const buffer = new ArrayBuffer(44 + numSamples), view = new DataView(buffer);
...
export function unlockAudioPlayback(): void {
    ...
    if (ctx && ctx.state === "suspended") {
        ...
        ctx.resume().then(() => { if (!keepRunning && !_activeGain && !_callAudioSessionActive) { try { void ctx.suspend(); } catch {} } }).catch(() => {});
    }
    if (_audioUnlocked) return;
    const audio = getSharedAudio(), url = silentWavUrl();
    audio.muted = true; audio.src = url;
    const finish = () => { try { audio.pause(); audio.currentTime = 0; } catch {} audio.muted = false; URL.revokeObjectURL(url); _audioUnlocked = true; };
    try { const p = audio.play(); if (p && typeof p.then === "function") p.then(finish).catch(() => { audio.muted = false; URL.revokeObjectURL(url); }); else finish(); } catch { audio.muted = false; URL.revokeObjectURL(url); }
}
...
function playAudioBlobElement(blob: Blob): { promise: Promise<void>; abort: () => void } {
    const url = URL.createObjectURL(blob), audio = getSharedAudio();
    audio.muted = false; audio.volume = _ttsVolume; audio.src = url;
    let settled = false, resolveFn: () => void = () => {};
    const finalize = () => {
    ...
    const promise = new Promise<void>(resolve => { resolveFn = resolve; audio.onended = finalize; audio.onerror = finalize; audio.play().catch(finalize); });
...
export function playAudioBlob(blob: Blob): { promise: Promise<void>; abort: () => void } {
    ...
    let settled = false, resolveFn: () => void = () => {}, source: AudioBufferSourceNode | null = null, gain: GainNode | null = null, fallbackAbort: (() => void) | null = null;
    const cleanupWebAudio = () => {
        if (source) { source.onended = null; try { source.stop(); } catch {} try { source.disconnect(); } catch {} source = null; }
        if (gain) { try { gain.disconnect(); } catch {} }
        if (_activeGain === gain) _activeGain = null;
        gain = null;
        try { void ctx.suspend(); } catch {}
    };
    const finalize = () => { if (settled) return; settled = true; cleanupWebAudio(); if (fallbackAbort) { fallbackAbort(); fallbackAbort = null; } resolveFn(); };
    const promise = new Promise<void>(resolve => {
        resolveFn = resolve;
        (async () => {
            try {
                if (ctx.state === "suspended") await Promise.race([ctx.resume().catch(() => {}), new Promise(r => setTimeout(r, 800))]);
                ...
                gain = ctx.createGain(); gain.gain.value = _ttsVolume; source.connect(gain); gain.connect(ctx.destination); _activeGain = gain;
                source.onended = finalize; source.start();
            } catch {
                if (settled) return;
                cleanupWebAudio();
                const fallback = playAudioBlobElement(blob); fallbackAbort = fallback.abort;
                void fallback.promise.then(() => { if (settled) return; settled = true; fallbackAbort = null; resolveFn(); });
            }
        })();
    });
    return { promise, abort: finalize };
...
export async function playTtsBlob(blob: Blob): Promise<void> {
    const result = playAudioBlob(blob);
